...
> 最後更新：2026-02-24 by Codex (GPT-5)
...
```
* b98bdf7 chore: normalize patch folder names and track patch3 files
* 00ae7ca docs: log git push and netlify tmp deploy
* 4f3619b docs: log smoke test and ntuh lookup timeout
...
- [2026-02-24] Codex (GPT-5): 正規化 patch 資料夾命名並清理 git 狀態：將舊版追蹤路徑 `patch update 0129/`、`Patch1/`、`Patch2/` 以 `R100` 方式移轉為 `Patch 0/`、`Patch 1/`、`Patch 2/`，並將 `Patch 3/`（`drug_data.js`、`note.md`）正式納入版控。chore commit `b98bdf7`。
- [2026-02-23] Codex (GPT-5): 依使用者要求執行 git 收尾與同步：`git fetch --all --prune` 後確認僅 `main`/`origin/main`，無其他本地或遠端 branch 可 merge；推送 `main` 至 GitHub（`4275e7d..4f3619b`）。
- [2026-02-23] Codex (GPT-5): Netlify production deploy 初次從 OneDrive 專案路徑執行失敗（Netlify CLI 寫 `.netlify/netlify.toml` 時 `ETIMEDOUT`）；改以 `/tmp` staging 目錄部署並排除 `.env`/`.env.age`/`age-key.txt`/`secrets/` 等敏感檔。部署成功：deploy id `699ce7f3021b99e8df59a61a`，production URL `https://ntuh-pharm-game.netlify.app`。
